...
async fn save_image(id: i64, bytes: &[u8]) -> anyhow::Result<()> {
    // Check that images folder exists and is a directory
...
fn make_thumbnail(id: i64) -> anyhow::Result<()> {
    ...
    let thumbnail = image.thumbnail(100, 100).into_rgb8();
...
async fn search_images(
    ...
    let rows = sqlx::query_as::<_, ImageRecord>("SELECT id, tags FROM images WHERE tags LIKE ?")
        .bind(tag)
        .fetch_all(&pool)
        .await
        .unwrap();
